...
function whoWon() {
    ...
    if (dieSum <= 5){
      balance -= 5;
      outcome = "You lose, please pay me " + Math.abs(balance) + " dollars.";
    } else if (dieSum >= 9){
      balance += 5;
      outcome = "You win, I pay you " + Math.abs(balance) + " dollars.";
    } else {
      outcome = "Its a draw, nobody wins or loses.";
      if(balance < 0){
        outcome += "\nYou owe me " + Math.abs(balance) + " dollars";
    } else {
        outcome += "\nI owe you " + Math.abs(balance) + " dollars";
    }
    }
...
function letsPlay(){
  rollDice();
  whoWon();
  var games = prompt("How many times would you like to play?");
  playGames(games);
}
...
function playGames(games){
  ...
  document.getElementById("playtime").innerHTML = "Played " + games + " times";
}
